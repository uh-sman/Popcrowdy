import React from "react";
import image from "../assets/Image-one.png";
import img1 from "../assets/Image-one.png";
import img2 from "../assets/Rectangle2.png";
import img3 from "../assets/Rectangle3.png";
import img4 from "../assets/Rectangle4.png";
import img5 from "../assets/Rectangle5.png";
import img6 from "../assets/Rectangle6.png";
import "react-responsive-carousel/lib/styles/carousel.min.css"; // requires a loader
import { Carousel } from "react-responsive-carousel";
import "./carousel.css";
const CarouselDemo = () => {
  const {
    changeSlide,
    setChangedSlide,
  } = (e) => {
    // const index = style={index:"-1"}
    setChangedSlide(-1);
  };
  return (
    <div>
      <Carousel onClickItem={changeSlide}>
        <div>
          <img src={image} className="sized-image" />
        </div>
        <div>
          <img src={img2} className="sized-image" />
        </div>
        <div>
          <img src={img3} className="sized-image" />
        </div>
        <div>
          <img src={img4} className="sized-image" />
        </div>
        <div>
          <img src={img5} className="sized-image" />
        </div>
        <div>
          <img src={img6} className="sized-image" />
        </div>
      </Carousel>
    </div>
  );
};

export default CarouselDemo;
